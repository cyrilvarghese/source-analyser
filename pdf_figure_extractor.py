...
def extract_figures_from_pdf(input_pdf_path: str, output_dir: str = None):
    """
    Extract figures, tables, and page images from a PDF using Docling.
    
    Args:
        input_pdf_path: Path to the input PDF file
        output_dir: Output directory (defaults to PDF name + '_extracted')
    """
    ...
    if output_dir is None:
        output_dir = f"{input_doc_path.stem}_extracted"
    ...
    try:
        # Convert the PDF
        _log.info("Converting PDF and extracting content...")
        ...
        _log.info(f"Pictures extracted: {picture_counter}")
        _log.info(f"Output directory: {output_dir}")
...
def main():
    parser = argparse.ArgumentParser(description="Extract figures from PDF using Docling")
    ...
    if args.list or not args.pdf_path:
        ...
        if not args.pdf_path and pdf_files:
            _log.info("\nTo extract figures, run:")
            _log.info("python pdf_figure_extractor.py <pdf_filename>")
            _log.info("\nFor example:")
            _log.info(f"python pdf_figure_extractor.py {pdf_files[0].name}")
        return
    
    # Extract figures from the specified PDF
    success = extract_figures_from_pdf(args.pdf_path, args.output)
    
    if success:
        _log.info("Figure extraction completed successfully!")
    else:
        _log.error("Figure extraction failed!")
        sys.exit(1)
